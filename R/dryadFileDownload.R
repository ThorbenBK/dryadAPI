dryadFileDownload <- function(doi){
  ...
  chosen_id <- id[choice]
  response2 <- GET(paste("https://datadryad.org/api/v2/versions/",chosen_id,"/files", sep=""))
  ...
  choice <- menu(file_names, title = "Choose a file to download/load")
  if (choice == 0) {
    cat("No file selected. Exiting.\n")
    return(NULL)
  }
